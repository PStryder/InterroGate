"""InterroGate's binding to the shared MetaGate bootstrap client.
...
InterroGate-specific thing: which primitive types map to which settings.
...
_BINDING_SPECS: tuple[tuple[str, str], ...] = (
    ("receiptgate", "receiptgate_url"),
    ("memorygate", "memorygate_url"),
)

COMPONENT_KEY = "interrogate"
...
async def bootstrap_from_metagate(settings: Any) -> Any:
    """Resolve InterroGate's peer endpoints from MetaGate."""
    if _shared is None:
        logger.warning(
